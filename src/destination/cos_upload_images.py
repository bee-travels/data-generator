...
def upload_all_files(bucket_name,folder_path):
    for folder in os.listdir("./"+folder_path):
        if isfile(folder):
            continue
        count = 0
        print("uploading data for :", folder)
        for file in os.listdir("./"+folder_path +"/"+ folder):
            count += 1
            file_path = f'./{folder_path}/{folder}/{file}'
            file_name = f'destination-{folder}-{count:03}.jpg'.lower()
...
def main():
    try:
        bucket_name = "bee-travels-destination"
        create_bucket(bucket_name)

        upload_all_files(bucket_name, "destination-images")
